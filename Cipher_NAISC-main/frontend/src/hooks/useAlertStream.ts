...
import { CipherMessageSchema } from '@/types/schemas'
import type { Alert, CipherMessage } from '@/types'
...
const RECONNECT_DELAYS = [1000, 2000, 3000] // max 3-second retry interval
...
export function useAlertStream() {
  ...
  const handleMessage = useCallback(
    (event: MessageEvent) => {
      ...
      const result = CipherMessageSchema.safeParse(raw)
      if (!result.success) {
        console.warn('[WS] Message failed schema validation:', result.error.flatten())
        return
      }

      const msg = raw as CipherMessage
